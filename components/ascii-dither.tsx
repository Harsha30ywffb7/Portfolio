...
export function AsciiDither({ src, cols = 90, color = '#6b5ce7', threshold = 0, invert = false, fill = false, borderRight = false, darkMode = false, cover = false, saturation = 6, loopPauseMs = 0, binarySize = false, binarySizeScale = 0.85, filterGreen = false, filterBlue = false, pureColor = false, greyscale = false, rawColor = false, tintRGB, cropTop = false, offsetYSchedule, playbackRateSchedule, xOffsetBySrc, yOffsetBySrc, scale = 1, onEnded, playbackRate = 1, batched = false, maxRenderFps, className = '' }: Props) {
  ...
  useEffect(() => {
    ...
    const samplerCtxRaw = sampler.getContext('2d', { willReadFrequently: true });
    if (!ctxRaw || !samplerCtxRaw) return;
    const ctx = ctxRaw;
    const samplerCtx = samplerCtxRaw;
    ...
    function draw() {
      ...
      try {
        pixels = samplerCtx.getImageData(0, 0, sampleCols, sampleRows).data;
      } catch {
        return;
      }
  ...
  }, [src, cols, color, threshold, invert, fill, darkMode, borderRight, cover, saturation, loopPauseMs, binarySize, binarySizeScale, filterGreen, filterBlue, pureColor, greyscale, rawColor, tintRGB, cropTop, offsetYSchedule, playbackRateSchedule, xOffsetBySrc, yOffsetBySrc, scale, batched, maxRenderFps, onEnded, playbackRate]);
